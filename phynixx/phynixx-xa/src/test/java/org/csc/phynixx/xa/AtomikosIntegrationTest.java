...
public class AtomikosIntegrationTest {
	...
	public void setUp() throws Exception {
		...
		this.factory1 = new TestXAResourceFactory("RF1", new TmpDirectory("xa_test").assertExitsDirectory("rf1"),
				this.transactionManagerProvider.getTransactionManager());

		this.factory2 = new TestXAResourceFactory("RF2", new TmpDirectory("xa_test").assertExitsDirectory("rf2"),
				this.transactionManagerProvider.getTransactionManager());
	...
	public void testOnePhaseReadOnly() throws Exception {
		IPhynixxXAConnection<ITestConnection> xaCon = factory1
				.getXAConnection();
		...
		log.info(TestConnectionStatusManager.toDebugString());
		TestStatusStack statusStack = TestConnectionStatusManager
				.getStatusStack(conId);
	...
	public void testTwoPhaseReadOnly() throws Exception {
		...
		IPhynixxXAConnection<ITestConnection> xaCon1 = factory1
				.getXAConnection();
		con1 = xaCon1.getConnection();
		conId1 = con1.getConnectionId();

		IPhynixxXAConnection<ITestConnection> xaCon2 = factory2
				.getXAConnection();
	...
	public void testJoinedRollback() throws Exception {
		...
		IPhynixxXAConnection<ITestConnection> xaCon2 = factory1
				.getXAConnection();
		...
		Assert.assertEquals(
				1,
				TestConnectionStatusManager.getStatusStack(conId2).countStatus(
						TestConnectionStatus.ROLLEDBACK));
		Assert.assertEquals(
				1,
				TestConnectionStatusManager.getStatusStack(conId2).countStatus(
						TestConnectionStatus.RELEASED));
	}
	...
	public void testJoinedCommit() throws Exception {
		...
		Assert.assertEquals(
				0,
				TestConnectionStatusManager.getStatusStack(conId2).countStatus(
						TestConnectionStatus.PREPARED));
		Assert.assertEquals(
				1,
				TestConnectionStatusManager.getStatusStack(conId2).countStatus(
						TestConnectionStatus.COMMITTED));
		Assert.assertEquals(
				1,
				TestConnectionStatusManager.getStatusStack(conId2).countStatus(
						TestConnectionStatus.RELEASED));
	...
	public void testExplicitEnlistment1() throws Exception {
		...
		this.getTransactionManager().getTransaction()
				.enlistResource(xaCon.getXAResource());
	 ...
	 */
	@Test
	public void testEmptyCommit2() throws Exception {
		IPhynixxXAConnection<ITestConnection> xaCon1 = factory1.getXAConnection();
	...
	public void testCommit3Connections1RM() throws Exception {
		...
		IPhynixxXAConnection<ITestConnection> xaCon3 = factory1
				.getXAConnection();
		...
		TestStatusStack statusStack = TestConnectionStatusManager
				.getStatusStack(con3.getConnectionId());
		TestCase.assertTrue(statusStack != null);
		TestCase.assertTrue(statusStack.isCommitted());
	...
	public void testSuspend() throws Exception {
		...
		IPhynixxXAConnection<ITestConnection> xaCon3 = factory1
				.getXAConnection();
		EhCacheXAResourceProducer.registerXAResource("XXXX",
				factory1.getXAResource());
		...
		TestStatusStack statusStack1 = TestConnectionStatusManager
				.getStatusStack(con1.getConnectionId());
		...
		TestStatusStack statusStack2 = TestConnectionStatusManager
				.getStatusStack(con2.getConnectionId());
	...
	/**
	 *
	 * Suspending one XAConnection, open a second transactional branch and
	...
	/**
	 * a transaction context bound to TX1. TX 1 is suspended and  new TX2 is
	 * started. Any modifications on the transactional context is attached to
	 * TX1.
	 *
	 * Any rollback/commit on TX2 doesn't effect the transactional context.
	 *
	 * @throws Exception
	 */
	@Test
	...
	/**
	 *
	 * Suspend one local transaction and open a second transactional branch and
	 * therefore a second physical connection.
	 *
	 * remember: the connection are associated to a TX by the call of
	 * xaConn.getConnection
	 ...
	 */
	@Test
	public void testMixedLocalAndGlobalTransaction() throws Exception {
	 ...
	 * rolled back at least.
	 * 
	 * Therefore the transactional context is closed and a new global
	 * transaction can be start on this XAConncgtion
	 *
	 * @throws Exception
	 */
	@Test
	public void testMixedLocalGlobalTransactionNested3() throws Exception {
		...
		// con1 in local transaction. Transactional context has transactional
		// data
	...
	public void testDifferentTransactionBranches2() throws Exception {
		...
		XidWrapper xid1 = new XidWrapper(10, new byte[] { 0x10 },
				new byte[] { 0x10 });
		XidWrapper xid2 = new XidWrapper(10, new byte[] { 0x20 },
				new byte[] { 0x20 });
		...
		try {
			...
			throw new AssertionFailedError(
					"A XAResource may no be associated to 2 active Transactions");
		} catch (Exception e) {
		}
