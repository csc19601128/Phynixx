...
/**
 * Logger uses a
 * {@link org.csc.phynixx.loggersystem.logger.channellogger.TAEnabledRandomAccessFile}
...
public class FileChannelDataLogger implements IDataLogger {
	...
	private static final IPhynixxLogger LOG = PhynixxLogManager
			.getLogger(FileChannelDataLogger.class);
	...
	static class FileAccessor {
		...
		private void instanciateFile() {
			...
			if (!cachedFile.exists()) {
				throw new IllegalStateException("File " + this.absolutePathName
						+ " does not exist");
			}
		}
		...
		public String toString() {
			return "FileAccessor {" + "cachedFile=" + cachedFile
	...
	private static final IPhynixxLogger LOGGER = PhynixxLogManager
			.getLogger(FileChannelDataLogger.class);
	...
	/**
	 * Opens a logger on base of die given logfile. A RandomAccessFile is
	 * created .
	 *
	 * The accessMode of the logger is
	 * {@link org.csc.phynixx.loggersystem.logger.channellogger.AccessMode#WRITE}
	 ...
	 * @throws IOException
	 */
	public FileChannelDataLogger(File logFileAccess, AccessMode aceessMode)
			throws IOException {
		this.logFileAccess = new FileAccessor(logFileAccess);
		String filename = this.logFileAccess.getAbsolutePathName();
		if (!new File(filename).exists()) {
			if (LOG.isInfoEnabled()) {
				FileChannelDataLoggerStatistics.notifyCreate(filename);
			}
		}
	...
	private void maybeWritten() {
		...
		if (this.accessMode != AccessMode.APPEND
	...
	/**
	 * opens the logger with the specified ACCESS_MODE. If the logger isn't
	 * closed it is closed
	 * 
	 * @param accessMode
	 * @throws IOException
	...
	private void associatedRandomAccessFile() throws IOException {
		...
		RandomAccessFile raf = openRandomAccessFile(
				this.logFileAccess.getFile(), FILE_MODE);
		try {
			this.randomAccess = new TAEnabledRandomAccessFile(
					this.logFileAccess.getFile(), raf);
			LOG.error(Thread.currentThread() + " lock on " + this.logFileAccess
					+ " succeeded");
		} catch (DelegatedRuntimeException e) {
			LOG.error(
					Thread.currentThread() + ".lock on " + this.logFileAccess,
					e);
			throw e;
	...
	public void reopen(AccessMode accessMode) throws IOException {
		...
		case APPEND:
			...
			break;
		default:
			throw new IllegalArgumentException("Invalid AccessMode "
					+ accessMode);
		}

	}

	private RandomAccessFile openRandomAccessFile(File logFile, String fileMode)
			throws IOException {
		try {
			RandomAccessFile raf = new RandomAccessFile(logFile, fileMode);
			if (LOG.isInfoEnabled()) {
				LOG.info(Thread.currentThread()	+ " Create RandomAccessFile on " + logFile	+"(Mode="+fileMode +") succeeded");
			}
			return raf;
		} catch (IOException e) {
			LOG.error(Thread.currentThread()
					+ " Create RandomAccessFile lock on " + logFile, e);
			throw e;
		} catch (IllegalStateException e) {
			LOG.error(Thread.currentThread()
					+ " Create RandomAccessFile lock on " + logFile, e);
	 ...
	 *
	 * @param type
	 *            a record type defined in LogRecordType.
	 * @param records
	...
	public void close() throws IOException {
		...
		String filename = this.logFileAccess.getAbsolutePathName();
		String logMessage = Thread.currentThread() + " " + filename + " closed";
		try {
			...
			this.logFileAccess.closeQuitly();
			this.randomAccess = null;
			LOG.error(logMessage);
			if (LOG.isInfoEnabled()) {
				FileChannelDataLoggerStatistics.notifyClose(filename);
			}
	...
	public void destroy() throws IOException {
		...
		try {
			...
			if (this.logFileAccess != null) {
				// try {
				// Files.delete(this.logFileAccess.getFile().toPath());
				// LOGGER.error("deleting "+this.logFileAccess.getFile());
				// } catch (Exception e) {
				// LOGGER.fatal("Failed deleting "+this.logFileAccess.getFile(),e);
				// // just log the error -- if cleanup fails, no failure of the
				// system
				...
				try {
					if (!this.logFileAccess.getFile().delete()) {
						...
						LOGGER.info("deleting "+this.logFileAccess.getFile());
					}
				} catch (Exception e) {
					LOGGER.fatal("Failed deleting "
							+ this.logFileAccess.getFile());
				}
				if (LOG.isInfoEnabled()) {
					FileChannelDataLoggerStatistics.notifyDestroy(filename);
				}
			}
		}
	}
	...
	public String toString() {
		return "FileChannelDataLogger ("
				+ this.logFileAccess.getAbsolutePathName() + ")";
	}
