...
public class MTIntegrationScenariosIT extends TestCase {
    ...
    private class WorkOnConnection implements IWorkOnConnection {
        ...
        public void work(IReferenceConnection con) throws InterruptedException {
            ...
            try {
                ...
                MTIntegrationScenariosIT.this.logger.info("Interrupted " + con.getId());
                throw e;
            }
            if (rd % 2 == 1) {
                con.rollback();
    ...
    private class Runner implements Runnable {
        ...
        public void run() {
            ...
            while (!isKilled() && (++cc) < NUMBER_OPF_TRIALS) {
                try {
                    con = (IReferenceConnection) MTIntegrationScenariosIT.this.factory.getConnection();
                    try {
                        MTIntegrationScenariosIT.this.workOnConnection.work(con);
                    } catch (InterruptedException e) {
                    }
                } finally {
                    if (con != null) {
